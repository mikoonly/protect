...
def prouser(func):
    async def function(app, message):
        ...
        babu = await get_prem()
        text = "<b>hi {message.from_user.mention}\n\n bot ini bisa menghapus pesan broadcast yang mengganggu di group,mau aktifin Anti-Gcast nya? silahkan hubungi owner dibawah ya</b> !!\n\n**Owner :**\n"
        count = 0
        for x in OWNER_ID:
            try:
...
def cekprem(func):
    async def function(app, message):
        ...
        text = "<b>hi {message.from_user.mention}\n\n bot ini bisa menghapus pesan broadcast yang mengganggu, mau aktifin Anti-Gcast nya? silahkan hubungi owner dibawah ya..</b> !!\n\n**Owner :**\n"
        count = 0
        for x in OWNER_ID:
            try:
                user = await app.get_users(x)
